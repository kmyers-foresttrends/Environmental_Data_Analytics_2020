...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean) #drop_na and na.omit remove any rows with NA, drop_na just removes NAs from a certain field
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365) # selecting 8th column, daily data, organizing data into time series format
# de-trending, looking at variability without the trend
# need to have data formatted in chronological order (ascending)
```

The `stl` function decomposes the time series object into its component parts. We must specify that the window for seasonal extraction is either "periodic" or a specific number of at least 7. The decomposition proceeds through a loess (locally estimated scatterplot smoothing or moving average) function.

```{r}
?stl
# Generate the decomposition
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic") # s.wdindow species time interval, e.g. 365 means daily, periodic mean default

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed) # trend is not monotonic, years on x axis because "repeat" is 365 days
# grey bar shows relative difference in axes
# remainder plot is similar to raw data bc seasonal and trend plots explain so little of the data
# lots of noise in seasonal signal, seasons are identical, repeated each year
# seasonal plot shows variance explained by seasons
# seasons means repeating cycle of 365 unites (can adjust this)
# choosing the frequency is just number of rows to repeat thru -> hard to interpret gaps
...
```{r}
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000) # bc tests don't like >5000 samples
EnoDischarge.late.subsample <- sample_n(EnoDischarge.late, 5000)

shapiro.test(EnoDischarge.early.subsample$discharge.mean) # non-normal
shapiro.test(EnoDischarge.late.subsample$discharge.mean) # non-normal

var.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean)

wilcox.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean) # non-parametric t-test
summary(EnoDischarge.early.subsample$discharge.mean)
summary(EnoDischarge.late.subsample$discharge.mean)
```

How might you interpret the results of this test, and how might you represent them graphically?

> The average discharges before and after 1985 are significantly different (Wilcoxon test, W = 1.17*10^8, p < 0.0001).Pre-1985, the mean discharge was 59.65 XX, whereas after 1985 it was 56.22.  To illustrate this difference, I would generate a boxplot of the distribution of discharges in each period.
...
NCAir.Garinger$Ozone <- na.approx(NCAir.Garinger$Ozone) # default interpolation is linear
...
EnoDischarge.late.monthly <- EnoDischarge.late %>%
  ...
  group_by(Year, Month) %>%
  summarise(Discharge = mean(discharge.mean)) # chose mean because it reflects high variability
  
EnoDischarge.late.monthly$Date <- as.Date(paste(EnoDischarge.late.monthly$Year, 
                                                EnoDischarge.late.monthly$Month, 
                                                1, sep="-"), # paste function synthesizes separate columns into date cells
                                          # don't use in time series!!
                                          format = "%Y-%m-%d")

# Generate time series (smk.test needs ts, not data.frame)
EnoDischarge.late.monthly.ts <- ts(EnoDischarge.late.monthly$Discharge, frequency = 12, # 12-monthly
                        start = c(1985, 10, 1), end = c(2019, 12, 1))
# Run SMK test
EnoDischarge.late.trend <- smk.test(EnoDischarge.late.monthly.ts)

# Inspect results
EnoDischarge.late.trend # no significant monotonic trend
...
> There is no significant trend in monthly discharge on the Eno River (Seasonal Mann-Kendall Test, p>0.05). Additionally, none of the months were individually significant. (report Z and p)
