...
# intercept is first plot, other significance of variables relative to intercept
...
Litter.Totals.groups # matching letters indicate insignificant difference
...
> Total litter biomass differed significantly amoung sites at the Niwot Ridge LTER (figure 1; ANOVAl F11,198 = 4.813, p<0.0001)
...
summary(Litter.anova.2way) # intercept is alphabetical first functional group and nlcd class
...
```{r}
# Format as aov
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass) # interaction syntax
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color=functionalGroup)) +
  ...
  ylim(0,10) +
  facet_wrap(Litter$functionalGroup, ncol=4) +
  labs(x="", y="Dry Mass (g)") +
  scale_x_discrete(labels=c("Evergreen","Grassland","Scrub")) +
  stat_summary(geom = "text", fun.y = max, vjust = 1, 
  size = 3.5,label = c("a", "b", "abcd", "bcd", "bcd", "a", 
                        "d", "abcd", "a", "b", "abcd", "bcd",
                       "bcd", "a", "d", "abcd","a", "b", "abcd",
                       "bcd", "bcd", "a", "d", "abcd"))
print(Litter.anova.plot)
