...
```{r}
# Vectors ---- # 4 dashes indicate collapsable code
...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # sorts sequence along columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # sorts sequence along rows
matrix2
...
```{r}
low <- c(31, 29, 43, 24, 23, 23, 26, 30, 42, 41)
high <- c(62, 45, 50, 50, 40, 40, 45, 52, 54, 58)
```
...
```{r}
forecast <- data.frame(low, high, lowC, highC) #
names(forecast) <- c("low (F)","high (F)","low (C)", "high (C)"); View(forecast)
```
...
```{r}
summary(forecast)
summary(forecast[['low (F)']])

sd(forecast) #doesn't work
sd(forecast[['low (F)']])
```
...
> Answer: masked tools message, for tidyverse all the difference packages load in
