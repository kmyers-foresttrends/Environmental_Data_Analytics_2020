...
```{r read.in.some.data}
basins_nf_seplains_raw <- st_read(here("Lessons","sf-lesson-20200303","data", "spatial_data", "bas_nonref_SEPlains.shp")) # read shapefile
...
# add your code here
# basins
st_crs(basins_nf_seplains_raw) <- my_proj4 # define datum first
st_crs(basins_nf_seplains_raw) <- my_epsg # then define projection
basins_nf_seplains <- basins_nf_seplains_raw # create new variable with new projection


# add your code here
# gages
st_crs(gages_raw) <- my_proj4 # define datum first
...
# add your code here
st_crs(southeast_state_bounds_raw) <- my_proj4 # define datum first
...
gages_tabular <- read_csv(here("Lessons","sf-lesson-20200303","data", "tabular_data", "gagesII_sept30_2011_tabular.csv")) %>%
...
# convert table to sf object using x and y data
...
pdf(here("Lessons","sf-lesson-20200303","outputs", "tabular_to_sf_check.pdf"), width = 11, height = 8.5)
ggplot() +
...
# add your code here
va_state_bound <-
  southeast_state_bounds %>%
  filter(NAME=="Virginia") %>%
  st_geometry() # drop attributes to decrease processing time

va_state_bound_geom <-
  southeast_state_bounds_geometry %>%
  filter(NAME=="Virginia")
class(va_state_bound)
...
# select watersheds that intersect with VA bounds
# add your code here
nc_basins_nf_seplains <- basins_nf_seplains_raw %>%
  st_intersection(va_state_bound)
...
```{r basic.sf.plotting}
# basic plot
pdf(here("Lessons","sf-lesson-20200303","outputs", "southeast_state_bounds.pdf"), width = 11, height = 8.5)
ggplot() +
...
# adding labels
pdf(here("Lessons","sf-lesson-20200303","outputs", "southeast_state_bounds_with_labs.pdf"), width = 11, height = 8.5)
...
pdf(here("Lessons","sf-lesson-20200303","outputs", "va_state_bound.pdf"), width = 11, height = 8.5)
ggplot(data = va_state_bound_geom) +
  geom_sf(aes(fill = Shape_Area)) +
  geom_sf_text(aes(label = NAME), color = "white") +
  theme_bw()  # not working, need sfc
dev.off()


# plot NC watershed boundaries from 3.1.2
pdf(here("Lessons","sf-lesson-20200303","outputs", "spatial_operation_test_fig.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data=va_state_bounds_geom) +
  geom_sf(data=va_basins_nf_seplains, fill="pink",alpha=0.25) +
  geom_sf(data=va_gaegs, color="black")
dev.off()
```
...
pdf(here("Lessons","sf-lesson-20200303","outputs", "spatial_operations_test_fig.pdf"), width = 11, height = 8.5)
# add your code here
ggplot(data = va_state_bound_geom) +
  geom_sf(aes(fill = Shape_Area)) +
  geom_sf_text(aes(label = NAME), color = "white") +
...
pdf(here("Lessons","sf-lesson-20200303","outputs", "nc_without_basemap.pdf"), width = 11, height = 8.5)
ggplot() +
...
# plot with basemap
# windows version?
pdf(here("Lessons","sf-lesson-20200303","outputs", "nc_with_basemap.pdf"), width = 11, height = 8.5)
ggmap(nc_basemap) + # didn't change crs, bc using ggmap, corrects on the fly?
...
# make figure 1b
# find watersheds that overlap with SC (want the whole watershed, not just the part of the watershe that's inside the SC bounds)
sc_basins_nf_seplains <- basins_nf_seplains %>%
  st_within(sc_state_bounds_geom)

# look at result
class(sc_basins_nf_seplains)

# make a function to convert binary to logic
sc_basins_nf_seplains$indicator <-  sc_basins_nf_seplains %>% lengths > 0

# map function on data and select only true values
basins_nf_seplains_in_sc <- basins_nf_seplains %>%
  mutate(sc_basins_nf_seplains$indicator) %>%
  filter(sc_basins_nf_seplains$indicator==TRUE)

# look at result
class(basins_nf_seplains_in_sc)
...
# plot figure 1b
# add your code here
pdf(here("Lessons","sf-lesson-20200303","outputs", "basins_test.pdf"), width = 11, height = 8.5)
ggplot(basins_nf_seplains_in_sc) +
  geom_sf()
dev.off()
```
