...
sc_basins_overlap <- basins_nf_seplains[st_within(basins_nf_seplains, sc_state_bounds_geom) %>% lengths > 0,]

# look at result
sc_basins_overlap
class(sc_basins_overlap)

# use sc_basins_overlap to find gages inside
gages_sc_basins <- gages %>%
  st_intersection(sc_basins_overlap)
# stretch break...this might take a while to run

# convert all to WGS84
my_epsg <- 4326
st_crs(gages_sc_basins) <- my_epsg
st_crs(gages_sc) <- my_epsg
st_crs(sc_basins_overlap) <- my_epsg
st_crs(sc_state_bounds_geom) <- my_epsg
...
# plot figure 1b
# add your code here
pdf(here("Lessons","sf-lesson-20200303","outputs", "figure1b.pdf"), width = 11, height = 8.5)
ggplot(sc_basemap) +
  geom_sf(sc_state_bounds_geom, fill="") +
  geom_sf(sc_basins_overlap, alph=0.25) +
  geom_sf(gages_sc_basins)
dev.off()
```
