...
#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") # data set not in env

# Another option is to choose with your browser
# read.csv(file.choose()) #nav to file explorer
...
```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # removes rows with NAs
dim(USGS.flow.data)
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") # format indicates order in original dataset
```
