...
NTL.1999 <- filter(NTL.phys.data, year4 == 1999) #condition for integer

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.1990to1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 <= 1999 & year4 >= 1990)

```
Question: Why don't we filter using row numbers?

> Answer: rows might not be ordered by years, would also take longer to index whichever rows you want
...
NTL.temp <- arrange(NTL.phys.data, desc(temperature_C)) #NAs will always go to the bottom
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(as.Date(sampledate, format = "%m/%d/%y")))
class(NTL.phys.data.PeterPaul1$sampledate)
