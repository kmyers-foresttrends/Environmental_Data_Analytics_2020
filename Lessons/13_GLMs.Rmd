...
# ancova - formulation the same as anova, just have continuous and categorical variables
# main effects
...
```{r}
TempTest.mixed <- lme(data = NTL.summertemp, # lme - linear mixed effects
                     temperature_C ~ year4, 
                     random = ~1|lakename) # 1 value may vary depending on context
#random effect must be categorical
summary(TempTest.mixed) # p-value rounded down to 0
rsquared(TempTest.mixed)

# Compare the random effects model with the fixed effects model
TempTest.fixed <- gls(data = NTL.summertemp, # gls just generates slightly different output from glm, part of lme package
                      temperature_C ~ year4)
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C)) +
  ...
  geom_smooth(method="lm", se=FALSE) + # fixed effect line
  # make it look better
  labs(x = "", y = expression("Temperature " ( degree~C)), color = "") +
  theme(legend.spacing.x = unit(0, "cm")) # make labels in legend closer together
print(NTL.tempmodel)
```

Question: How would you interpret the collective results of your mixed effects model in the context of the study question?

> ANSWER: From 1984 to 2016, temperature increased 0.06 degrees Celsius annually (mixed effects model, R-squared = , p-value < ).  OVer this period, surface waters in north Wisconsin lakes increased a total of 1.8 degrees.
