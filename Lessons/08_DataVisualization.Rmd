...
temp_day <- ggplot(subset(PeterPaul.chem.nutrients, depth==0), aes(x=daynum, y=temperature_C)) +
  geom_point() + facet_wrap(vars(Site.Name), nrow = 3
  
print(temp_day)
# Color your points by year, and facet by lake in two rows.
temp_dayyear <- ggplot(subset(PeterPaul.chem.nutrients, depth==0)) +
  geom_point(aes(x=daynum, y=temperature_C, color = as.factor(year4))) + facet_wrap(vars(lakename), nrow = 2)
...
temp_date <- ggplot(PeterPaul.chem.nutrients) + 
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + # se = False removes confidence interval
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # alpha is transparency indicator, specify outside of aesthetics
print(Nutrientplot6)
summary(PeterPaul.chem.nutrients.gathered$lakename)
