...
getwd()
library(tidyverse)
options(scipen = 4) # displays 4 digits instead of exponential notation, applies to ggplot and stat test results
...
summary(irradiance.regression) # for every increase in m depth, there is -95 radiance
# adjusted R-squared
...
> With increasing depth, irradiance decreases.  
> Depth accounts for approximately 30% of irradiance decrease.  
> Irradiance decreases significantly with increasing depth (linear regression, R2 = 0.31, df=15,449, p < 0.0001).
> Include coefficients in table or text, not in parentheses.
...
par(mfrow = c(1,1)) # sets the plot window back to 1,1
# top left - want horizontal and no clustering, top right - data should fall along line
# bottom left - same as top left, bottom right = shows outliers
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) # remove 0s bc can't log transform
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2) # log transform to simulate linear relationship
...
# SE can also be removed, can't see in above plot because confidence is so high
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point() +
    scale_y_log10() +
    geom_smooth(method = 'lm', se = FALSE, color = "black") # default is loess model in method
print(irradiancebydepth2)

# Make the graph attractive
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha=0.5,pch=16,color="dodgerblue") +
    scale_y_log10() +
...
``` {r}
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth, 
         method = "spearman", exact = FALSE) # no R-squared value
```
...
corrplot(PeterPaulCorr, method = "ellipse") # color is strength of correlation, ellipse corresponds to color
...
summary(TPmodel) # n is not a significant predictor, but does account for significant amount of variance

```

> Our results reveal that dissolved oxygen, temperature, and nitrogen concentration can predict 26% of the concentration of phosphorus in Paul Lake (linear regression, R-squared=0.26, p < 0.0001).