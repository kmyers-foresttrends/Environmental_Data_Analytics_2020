...
EPAair.subsample <- sample_n(EPAair, 5000) # shapiro test can only take 5000 samples

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) # null hypothesis = normality
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram()  # distribution is right-skewed
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) # colon means continue code

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") # mu is null distribution mean, no alternative = two-tailed t-test, but we expect mu < 50, so we have a 'less than' alternative hypothesis
O3.onesample # tests null hypothesis that mean of groups = specific value
...
Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  ...
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) # starts axes at 0
print(Ozone.plot) # shows how distribution of ozone data lies to the left of 50
```
...
> The results of the one-way t-test show that the ozone levels from the EPA air data are within a 'good' range (less than 50).  This is because the distribution of the data was significantly less than a mean value of 50 (t-test, t=-57.98, p<0.0001).
...
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly() # this is good for exploratory analysis

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year)
O3.twosample
O3.twosample$p.value # null hypothesis rejected because sample size is high
...
```{r}
#
o3yr <- ggplot(EPAair, aes(y=Ozone, shape=as.factor(Year), fill=as.factor(Year))) +
  geom_density(color=NA, alpha=0.5) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))
o3yr

o3box <- ggplot(EPAair) +
  geom_boxplot(aes(y=Ozone, x=as.factor(Year), color=as.factor(Year))) #+
  #stat_summary()
o3box
