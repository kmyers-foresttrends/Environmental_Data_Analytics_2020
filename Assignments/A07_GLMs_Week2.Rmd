...
step(ntl_lm)
summary(ntl_lm)  # am i understanding this question correctly?
...
summary(ntl_int) # understanding this question?
```
...
> Answer: There is a significant interaction between depth and lake name which explains 76.5% of the variance in temperature.
...
tdepth <- ggplot(ntl_filter,aes(y=temperature_C,x=depth,color=lakename)) +
  ...
  ylim(0,35) +
  scale_color_manual(values = c("darkred", "red", "darkorange", "orange", "yellow","green","darkgreen","blue","purple")) +
  labs(y=expression("Temperature ("*degree*"C)"), x="Depth (m)",color="")
print(tdepth)  # have the axes assigned correctly?
```
...
library(nlme)
#9a
mem1 <- lme(data=neon, dryMass~nlcdClass:functionalGroup, random=~1|plotID) #getting error
#9b
```
...
```{r}
 mem2 <- lme(data=neon, dryMass~nlcdClass:functionalGroup)
aov(mem1, mem2)
```
